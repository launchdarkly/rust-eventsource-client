...
impl EventParser {
    ...
    fn parse_complete_lines_into_event(&mut self) -> Result<()> {
        loop {
            ...
            while let Some(mut line) = self.complete_lines.pop_front() {
                ...
                // Some SSM streams, like the ones generated by Ollama don't have a prefix key, and they just
                // start returning a JSON encoded payload. We need to be able to account for this scenario.
                if let Some(0) = line.iter().position(|&b| b'{' == b) {
                    let mut new_line = Vec::with_capacity(line.len() + 6);
                    new_line.extend_from_slice(b"data: ");
